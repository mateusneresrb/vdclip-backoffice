...
function isTaxActive(tax: TaxConfig): boolean {
  return tax.effectiveTo === null || new Date(tax.effectiveTo) > new Date()
}
          ...
          <CardContent>
            ...
            <div className="hidden md:block">
              <div className="space-y-1">
                <div className="grid grid-cols-6 gap-2 border-b px-3 py-2 text-[11px] font-medium text-muted-foreground">
                  <span>{t('finance.taxTable.name')}</span>
                  <span className="text-right">{t('finance.taxTable.rate')}</span>
                  <span>{t('finance.taxTable.regime')}</span>
                  <span>{t('finance.taxTable.effectiveFrom')}</span>
                  <span className="text-right">{t('finance.taxTable.active')}</span>
                  <span></span>
                </div>
                {pagination.paginatedItems.map((tax) => {
                  const active = isTaxActive(tax)
                  return (
                    <div key={tax.id} className="grid grid-cols-6 items-center gap-2 rounded-md px-3 py-2.5 hover:bg-muted/50">
                      <span className="text-sm font-medium">{tax.taxType}</span>
                      ...
                      <span className="text-xs text-muted-foreground">{tax.effectiveFrom}</span>
                      <div className="flex justify-end">
                        <span className={cn('inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium', active ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' : 'bg-slate-500/15 text-slate-700 dark:text-slate-400')}>
                          {active ? t('finance.taxActive') : t('finance.taxInactive')}
                        </span>
                      </div>
                      <div className="flex justify-end">
                        <DropdownMenu>
                  ...
                  <div key={tax.id} className="flex items-center justify-between rounded-lg border p-3">
                    <div className="min-w-0 flex-1 space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        <span className="text-[10px] text-muted-foreground">{tax.effectiveFrom}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <DropdownMenu>
