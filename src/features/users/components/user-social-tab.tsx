...
import { useUserSocialConnections } from '../hooks/use-user-social-connections'
...
function PlatformIcon ({ platform }: { platform: string }) {
  ...
  if (platform === 'instagram') {
    return (
      <svg viewBox="0 0 24 24" className="size-5 shrink-0" fill="url(#ig-grad)">
        <defs>
...
export function UserSocialTab({ userId }: { userId: string }) {
  const { t } = useTranslation('admin')
  const { data: connections, isLoading } = useUserSocialConnections(userId)
